...
//extern char bill_info[500];
//extern unsigned int bill_lenIdx ;
...
//	uart_UBBR =  F_CPU/(uart_cfg->baudrate*16);
	uart_UBBR =  53;
...
BOOLEAN EF_BOOLEAN_UART_GetChar(U8_t * returnedValue)
{
	...
	/* make UART unstuck*/
    while( (!(*uart_reg.UCSRA_Reg & (1 << RXC))) && (!EF_BOOLEAN_Timer_IsTimedOut(UART_TIMER_ID)) );
    /* extracting from the previous loop because of receiving byte or Time out*/

    if (EF_BOOLEAN_Timer_IsTimedOut(UART_TIMER_ID))
    {
    	 reciveFlag   = FALSE;
    ...
    else
    {
    	...
    	EF_void_UART_SendString("recived flag ");
    	EF_void_UART_Send_Integer(reciveFlag);
...
*
******************************************************************************/
void EF_void_UART_SendArray(char *array, int Length)
{
    int ArrayIndex;
...
//ISR(USART_RXC_vect)
//{
//	bill_info[bill_lenIdx] = UDR;
//	bill_lenIdx++;
////	EF_void_UART_SendString("data length ");
////	EF_void_UART_Send_Integer(bill_lenIdx);
//// EF_void_UART_SendString("\n");
//
//}
...
{
	U8_t index = 0 ;
	volatile U8_t b_Status = 0 ;

	U8_t ReceviedByte =0;
	do
	{
		b_Status = EF_BOOLEAN_UART_GetChar(  &ReceviedByte );
	...
	}while ( *(ReturnedArray + index-1) != ByteToStop );
